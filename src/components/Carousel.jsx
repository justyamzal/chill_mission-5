...
export default function Carousel({
  ...
  return (
    <section className="w-full px-5 md:px-20 py-5 md:py-10">
      {title && <h3 className="text-2xl md:text-[32px] font-bold pb-5 md:pb-8">{title}</h3>}

      <Splide aria-label={title} options={options}>
        {items.map((item, i) => (
          <SplideSlide key={i}>
            {history ? (
              <div className="relative w-[302px] h-[162px] cursor-pointer">
                <img src={item.src} alt={item.title || ""} className="w-full h-full rounded-md object-cover" />
                <div className="absolute inset-x-0 bottom-0 p-4 flex items-center justify-between rounded-md"
                     style={{ background: "linear-gradient(rgba(0,0,0,0), rgba(0,0,0,1))" }}>
                  <div className="font-bold text-lg leading-tight">{item.title}</div>
                  <div className="flex items-center gap-2 text-sm">
                    <img src="../public/icon/star.svg" alt="" className="w-4 h-4" />
